    ...
    async (args) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          return {
            content: [
              { type: "text" as const, text: `Ollama error (${res.status}): ${errorText}` },
            ],
            isError: true,
          };
        }
        ...
        return {
          content: [
            { type: "text" as const, text: `Pull complete: ${args.model} — ${data.status}` },
          ],
        };
      } catch (err) {
        return {
  ...
  server.tool(
    ...
    async () => {
      ...
      try {
        ...
        if (models.length === 0) {
          return {
            content: [{ type: "text" as const, text: "No models currently loaded in memory." }],
          };
        }
        const list = models
          .map((m) => {
